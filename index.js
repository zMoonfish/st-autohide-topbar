...
(function () {
    console.log("[auto-hide] file loaded");

    const ev = window.eventSource;
    const types = window.event_types;

    if (!ev || !types) {
        console.log("[auto-hide] ST globals not found");
        return;
    }

    ev.on(types.APP_READY, () => {
        console.log("[auto-hide] APP_READY");

        const TOP_BAR = document.getElementById("top-bar");
        const SETTINGS = document.getElementById("top-settings-holder");

        if (!TOP_BAR || !SETTINGS) {
            console.log("[auto-hide] elements missing");
            return;
        }

        const height = Math.max(TOP_BAR.offsetHeight, SETTINGS.offsetHeight);
